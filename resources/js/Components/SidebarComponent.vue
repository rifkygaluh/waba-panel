<template>
  <div class="pb-4">
    ...
    >
      <div class="sidebar-routes pt-5 bg-white">
        <InertiaLink :href="route('home')" class="px-2">
          <a-menu-item class="block h-fit" :key="route('home')">
            <HomeOutlined />
            <a-typography-text class="block whitespace-normal py-2">
              Home
            </a-typography-text>
          </a-menu-item>
        </InertiaLink>
...
const verificationPayload = {
  assignee_id: user.role.role_number === verificatorRole
    ? user.id
    : '',
};
Object.keys(verificationPayload).forEach((k) => verificationPayload[k] === '' && delete verificationPayload[k]);
</script>
<style>
.ant-menu {
...
li.ant-menu-item, div.ant-menu-submenu-title {
