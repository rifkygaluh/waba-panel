...
@section('content')
    <section class="container px-5 mx-auto pb-20 md:mt-[-20px] mt-[100px]">
        <div class="md:px-10">
            <div class="font-bold text-[24px] mb-5">{{ $title }}</div>
            <div id="blogContainer" class="grid md:grid-cols-4 grid-cols-1 gap-5"></div>
...
@section('script')
    <script>
        ...
        const url = "{{ url('blog/data') }}"+"?page={{$page}}&category={{$category}}&count="+dataPerPage;
        ...
        fetch(url)
            ...
            .then(data => {
                if(data.currentPage > data.totalPage){
                    window.location.href = "{{url('blog')}}"+"?category="+data.category+"&page="+data.totalPage;
                }
        ...
        function setPagination(data) {
            ...
            if (totalPage > 1) {
                const urlPrevious = "{{ url('blog?category=') }}"+category+"&page="+(currentPage > 1 ? currentPage - 1 : "#");
                ...
                const urlNext = "{{ url('blog?category=') }}"+category+"&page="+(currentPage == totalPage ? "#" : currentPage + 1);
                ...
                if (totalPage <= 5) {
                    ...
                    for (let i = 1; i <= totalPage; i++) {
                        paginationContainer.append($(pagesNumber).text(i).attr("href", "{{url('blog?category=')}}"+category+"&page="+i).addClass(i == currentPage ? 'bg-[var(--waba-primary-color)]' : ''));
                    }
                } else {
                    if (currentPage <= 3) {
                        ...
                        paginationContainer.append($(pagesNumber).text(totalPage).attr("href", "{{url('blog?category=')}}"+category+"&page="+totalPage).addClass(i == currentPage ? 'bg-[var(--waba-primary-color)]' : ''));
                    } else if (currentPage >= totalPage - 2) {
                        paginationContainer.append($(pagesNumber).text(1).attr("href", "{{url('blog?category=')}}"+category+"&page="+1).addClass(i == currentPage ? 'bg-[var(--waba-primary-color)]' : ''));
                        paginationContainer.append($(separator).text('...'));
